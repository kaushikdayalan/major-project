...
class adminStausUpdateDocsInOut extends Component{
      ...
      if(data.error){
        ...
        .then(data=>{
          if(data.error){
            ...
            this.setState({documentList:data,available:true});
  ...
  updateDocsIn = event=>{
    ...
    render(){
        const {error,message,documentList,fileName,available,docListError} = this.state
        return(
            <div>
            <AdminMenu/>
        ...
        <div className="row justify-content-md-center">
          <div className="col-sm-5">
          <div className="alert alert-primary" style={{display: docListError?"":"none"}}>
                    {docListError}
                </div>
          </div>
        </div>
